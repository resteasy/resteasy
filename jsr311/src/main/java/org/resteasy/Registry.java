...
public class Registry {
    ...
    public class Node
    {
        ...
        public void addChild(String[] path, int pathIndex, ResourceInvoker invoker)
        {
            ...
            if (matcher.matches())
            {
                String uriParamName = matcher.group();
        ...
        public ResourceInvoker findResourceInvoker(String[] path, int pathIndex, MediaType contentType, List<MediaType> accepts)
        {
            if (pathIndex >= path.length) return matchByType(contentType, accepts);
            else return findChild(path, pathIndex, contentType, accepts);
        }

        private ResourceInvoker findChild(String[] path, int pathIndex, MediaType contentType, List<MediaType> accepts) {
            Node next = children.get(path[pathIndex]);
            if (next != null) return next.findResourceInvoker(path, ++pathIndex, contentType, accepts);
            else if (wildChildren != null)
            {
               for (Node wildcard : wildChildren)
               {
                   ResourceInvoker wildcardReturn = wildcard.findResourceInvoker(path, ++pathIndex, contentType, accepts);
        ...
        private ResourceInvoker matchByType(MediaType contentType, List<MediaType> accepts)
        {
            for (ResourceInvoker invoker : invokers)
            {
                if (invoker.matchByType(contentType, accepts)) return invoker;
            }
            return null;
        }
    ...
    public void addResourceFactory(ResourceFactory factory)
    {
        ...
        for (Method method : clazz.getMethods())
        {
            ...
            if (path != null) pathExpression = (pathExpression == null) ? path.value() : pathExpression + "/" + path.value();
    ...
    public ResourceInvoker getResourceInvoker(String path, MediaType contentType, List<MediaType> accepts)
    {
        if (path.startsWith("/")) path = path.substring(1);
        return root.findResourceInvoker(path.split("/"), 0, contentType, accepts);
    }
}
