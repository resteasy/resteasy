...
public class BasicTest extends BaseResourceTest
{
   ...
   public void testTasks() throws Exception
   {
      ...
      processEngine.getIdentityService().createUser("peter", "Peter", "Pan");
      processEngine.getIdentityService().createGroup("user");
      processEngine.getIdentityService().createMembership("peter", "user");
      processEngine.getIdentityService().createUser("alex", "Alex", "Ander");
      processEngine.getIdentityService().createGroup("manager");
      processEngine.getIdentityService().createMembership("alex", "manager");
      String mediaType = "bpm/bpmn";
      String file = "bpmn/tasks.bpmn.xml";
      InputStream jpdl = Thread.currentThread().getContextClassLoader().getResourceAsStream(file);
      Assert.assertNotNull(jpdl);
      ...
      request = executor.createRequest("http://localhost:8081/bpm/tasks/next?user=peter");
      response = request.post();
      Assert.assertEquals(response.getStatus(), 200);

      System.out.println("LinkHeader: " + response.getLinkHeader());
      Link requestVacation = response.getLinkHeader().getLinkByTitle("request-vacation");
      Assert.assertNotNull(requestVacation);
      response = requestVacation.request().post();
      Assert.assertEquals(204, response.getStatus());

      request = executor.createRequest("http://localhost:8081/bpm/tasks/next?user=alex");
      response = request.post();
      System.out.println("LinkHeader: " + response.getLinkHeader());
      Link notOk = response.getLinkHeader().getLinkByTitle("notOk");
      response = notOk.request().post();
   ...
   @Test
   public void testTasksWithVariables() throws Exception
   {
      ...
      processEngine.getIdentityService().createMembership("alex", "manager");
      String mediaType = "bpm/bpmn";
      String file = "bpmn/tasksWithVariables.bpmn.xml";
      InputStream jpdl = Thread.currentThread().getContextClassLoader().getResourceAsStream(file);
      Assert.assertNotNull(jpdl);
      ...
      response = request.post();
      System.out.println("LinkHeader: " + response.getLinkHeader());
      Link complete = response.getLinkHeader().getLinkByTitle("complete");
      Assert.assertNotNull(complete);

      MultipartFormDataOutput form = new MultipartFormDataOutput();
      form.addFormData("verificationResult", "Not OK", MediaType.TEXT_PLAIN_TYPE);
