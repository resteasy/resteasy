...
public final class FindAnnotation {
    ...
    *
    */
    private static final Class<? extends Annotation>[] JAXRS_ANNOTATIONS = (Class<? extends Annotation>[]) new Class[] {
            QueryParam.class,
            HeaderParam.class,
            CookieParam.class,
            PathParam.class,
    ...
    public static <T> T findAnnotation(Annotation[] searchList, String... annotations) {
        if (searchList == null || annotations == null || annotations.length == 0)
            return null;
        for (Annotation ann : searchList) {
