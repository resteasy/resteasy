...
@ResteasyValidationCdiEnabledAnnotation
public class ResteasyValidationCdiInterceptor
{
   ...
   {
      log.trace("*** Intercepting call in ResteasyValidationCdiInterceptor.intercept()");
      ...
      if (isSetter(ctx) && !Boolean.parseBoolean(context.getInitParameter(VALIDATE_SETTERS)))
      {
         return invoke(ctx);
      }
      ...
      log.trace("*** Back from intercepting call in ResteasyValidationCdiInterceptor.intercept()");
      if (validator != null)
      {
   ...
   protected Object invoke(InvocationContext ctx) throws Exception
   {
      ...
      catch (ConstraintViolationException e)
      {
         ...
         result = e.getConstraintViolations().iterator().next().getExecutableReturnValue();
      }
      return result;
   }
}
