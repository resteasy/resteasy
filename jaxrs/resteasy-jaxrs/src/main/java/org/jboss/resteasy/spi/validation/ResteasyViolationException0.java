 ...
 * @TODO Add javadoc.
 */
public class ResteasyViolationException0 extends RuntimeException
{
   ...
   public ResteasyViolationException0()
   {
//      super(500);
   }

   public ResteasyViolationException0(ViolationsContainer<?> container)
   {
//      super(500);
   ...
   protected void convertToStrings()
   {
      ...
      Iterator<ResteasyConstraintViolation> it = container.getFieldViolations().iterator();
      while (it.hasNext())
      {
//         ResteasyConstraintViolation cv = (ConstraintViolation) it.next();
//         fieldViolations.add("field " + cv.getPropertyPath() + ": " + cv.getMessage() + ": " + cv.getInvalidValue().toString());
         fieldViolations.add(it.next().toString());
      }
...
//         propertyViolations.add("property " + cv.getPropertyPath() + ": " + cv.getMessage() + ": " + cv.getInvalidValue().toString());
         propertyViolations.add(it.next().toString());
      }
...
//         classViolations.add(cv.getMessage() + ": " + cv.getInvalidValue().toString());
         classViolations.add(it.next().toString());
      }
...
//         parameterViolations.add("parameter " + cv.getPropertyPath() + ": " + cv.getMessage() + ": " + cv.getInvalidValue().toString());
         parameterViolations.add(it.next().toString());
      }
...
//         returnValueViolations.add("return value: " + cv.getMessage() + ": " + cv.getInvalidValue().toString());
         returnValueViolations.add(it.next().toString());
      }
