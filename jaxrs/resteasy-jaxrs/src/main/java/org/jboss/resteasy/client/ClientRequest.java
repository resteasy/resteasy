...
public class ClientRequest extends ClientInterceptorRepositoryImpl{
	...
	public ClientRequest(String uriTemplate) {
		this(uriTemplate, new HttpClient(), ResteasyProviderFactory
				.getInstance());
	}
	...
	public ClientRequest body(String contentType, Object data) {
		return body(MediaType.valueOf(contentType), data, data.getClass(),
				null, null);
	}
	...
	public ClientResponse execute() throws Exception {
		if (getReaderInterceptorList().isEmpty())
			setReaderInterceptors(providerFactory
					.getClientMessageBodyReaderInterceptorRegistry().bindForList(null,
							null));
	...
	public void writeRequestBody(MultivaluedMap<String, Object> headers,
		...
		if (getWriterInterceptorList().isEmpty())
			setWriterInterceptors(providerFactory
					.getClientMessageBodyWriterInterceptorRegistry().bindForList(null,
							null));
		if (writer == null) {
			writer = providerFactory.getMessageBodyWriter(getBodyType(),
					getBodyGenericType(), getBodyAnnotations(),
					getBodyContentType());
		}
...

	{
		...
		if (pathParameters != null || pathParameterList != null ) {
			if(pathParameters == null){
				return finalUri = builder.build(pathParameterList.toArray()).toString();
			} else {
				for (Map.Entry<String, List<String>> entry : pathParameters
						.entrySet()) {
					List<String> values = entry.getValue();
					for (String value : values)
