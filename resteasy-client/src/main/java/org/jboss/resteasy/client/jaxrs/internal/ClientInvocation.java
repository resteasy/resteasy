...
public class ClientInvocation implements Invocation {
    ...
    // todo need a better solution for this.  Apache Http Client 4 does not let you obtain the OutputStream before executing this request. That is problematic for wrapping the output stream in e.g. a RequestFilter for transparent compressing.
    ...
    private <T> Future<T> executorSubmit(
            ExecutorService executor, final InvocationCallback<T> callback, final AsyncClientHttpEngine.ResultExtractor<T> extractor) {
