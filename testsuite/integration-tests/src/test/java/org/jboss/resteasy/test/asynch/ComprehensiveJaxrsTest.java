...
public class ComprehensiveJaxrsTest {
    ...
    public static Archive<?> createTestArchive() {
        ...
        war.addClasses(JaxrsAsyncServletXmlData.class, JaxrsAsyncServletAsyncResponseBlockingQueue.class, JaxrsAsyncServletJaxrsResource.class,
                JaxrsAsyncServletApp.class, JaxrsAsyncServletPrintingErrorHandler.class,
    ...
    private void suspendResumeTest() throws Exception {
        invokeClear();
    ...
    private void cancelVoidTest() throws Exception {
        invokeClear();
        Future<Response> suspend = invokeRequest("suspend");
    ...
    private void setTimeoutTest() throws Exception {
        invokeClear();
        logMsg("here 1");
        Future<Response> suspend = invokeRequest("suspend");
        logMsg("here 2");
    ...
    private void cancelDateTest() throws Exception {
        ...
        checkEquals(new Date(milis), retry, "Unexpected",
    ...
    private void cancelIntTest() throws Exception {
        String seconds = "20";
        invokeClear();
    ...
    public void complexTest() throws Exception {
        ...
        // resumeAnyJavaObjectInputStreamTest
        {
            invokeClear();
            String expectedResponse = "Expected response";
            Future<Response> suspend = invokeRequest("suspend");
        ...
        // resumeWithCheckedExceptionTest
        {
            invokeClear();
            Future<Response> suspend = invokeRequest("suspend");
        ...
        // resumeWithRuntimeExceptionTest
        {
            invokeClear();
            Future<Response> suspend = invokeRequest("suspend");
        ...
        // resumeWithExceptionReturnsFalseWhenResumedTest
        {
            suspendResumeTest();
    ...
    private static void checktStatus(Response response, Response.Status status)
            throws Exception {
        checkEquals(response.getStatus(), status.getStatusCode(),
                "Unexpected status code received", response.getStatus(),
                "expected was", status);
        logMsg("Found expected status", status);
    }

    private static void checkString(Future<Response> future, String check)
            throws Exception {
    ...
    private static void checkException(Future<Response> future,
                                       Class<? extends Throwable> e) throws Exception {
        String clazz = e.getName();
        Response response = getResponse(future);
        checktStatus(response, Response.Status.NOT_ACCEPTABLE);
        checkContainsString(response.readEntity(String.class), clazz, clazz,
                "not thrown");
        logMsg(clazz, "has been thrown as expected");
    }

    public static void checkContainsString(String string, String substring,
                                           Object... message) throws Exception {
        checkTrue(string.contains(substring), message);
    }

    public static <T> void checkContains(T text, T subtext, Object... message)
            throws Exception {
