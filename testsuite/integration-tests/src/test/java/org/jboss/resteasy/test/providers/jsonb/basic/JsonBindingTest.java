...
public class JsonBindingTest {
   ...
   public static Archive<?> deployWithJsonB() {
      return deploy(WAR_WITH_JSONB, true, false);
   }

   @Deployment(name = CUSTOM_JSON_PROVIDER)
   public static Archive<?> deployWithoutJsonB() {
      return deploy(CUSTOM_JSON_PROVIDER, false, false);
   }
   ...
   public static Archive<?> deploymentWithJsonBAsciiEncoding() {
      return deploy(WAR_WITH_JSONB_ASCII_ENCODING, true, true);
   }

   public static Archive<?> deploy(String archiveName, boolean useJsonB, boolean testEncoding) {
      ...
      if (testEncoding) {
         ...
         contextParams.put("org.jboss.resteasy.jsonb.disable.charset", "true");
         return TestUtil.finishContainerPrepare(war, contextParams, JsonBindingResource.class);
      }
      if (useJsonB) {
         war.addAsManifestResource("jboss-deployment-structure-json-b.xml", "jboss-deployment-structure.xml");
