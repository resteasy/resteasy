...
public class JsonBindingTest {
   ...
   public void jsonbOnClientTest() throws Exception {
      ...
      assertThat("Failed to return the correct name", response.getName(), is("Rosa"));
      assertThat("Variable with JsonbTransient annotation should be transient, if JSON-B is used",
   ...
   public void jsonbOnClientTestWithoutEncoding() throws Exception {
      ...
      assertThat("Failed to return the correct name", response.getName(), is("Graça"));
      assertThat("Variable with JsonbTransient annotation should be transient, if JSON-B is used",
              response.getTransientVar(), is(Cat.DEFAULT_TRANSIENT_VAR_VALUE));
   }
   ...
   public void negativeScenarioOnClient() throws Exception {
      ...
      assertThat("Server should use custom JSON provider", responseAsString, containsString(Cat.CUSTOM_TO_STRING_FORMAT));
      ...
      try {
         ...
         for (String stackTraceLine : stackTraceString.split(System.lineSeparator())) {
            assertThat("User-unfriendly error message in JSON-B", stackTraceLine,
