...
public interface Messages
{
   ...
   @Message(id = BASE + 20, value = "Failed to evaluate EL expression: %s")
   String failedToEvaluateELExpression(String expression);
   
   @Message(id = BASE + 25, value = "Failed to find bean property %s")
...
//   <T> String failedToInjectLinks(T expression);
   ...
   @Message(id = BASE + 40, value = "Failed to read field %s")
   String failedToReadField(String field);
   
   @Message(id = BASE + 45, value = "Failed to read property %s")
   String failedToReadProperty(String property);

   @Message(id = BASE + 50, value = "Failed to read property from method %s")
   String failedToReadPropertyFromMethod(String property);
   
   @Message(id = BASE + 55, value = " Not enough URI parameters: expecting {0} but only found {1}", format=Format.MESSAGE_FORMAT)
