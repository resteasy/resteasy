...
public interface Messages
{
   ...
   @Message(id = BASE + 45, value = "Found BeanManager at java:app/BeanManager")
   String foundBeanManagerAtJavaApp();

   @Message(id = BASE + 50, value = "Found BeanManager at java:comp/BeanManager")
   String foundBeanManagerAtJavaComp();

   @Message(id = BASE + 55, value = "Found BeanManager in ServletContext")
   String foundBeanManagerInServletContext();

   @Message(id = BASE + 60, value = "No CDI beans found for {0}. Using default ConstructorInjector.", format=Format.MESSAGE_FORMAT)
   String noCDIBeansFound(Class<?> clazz);

   @Message(id = BASE + 65, value = "No lookup interface found for {0}", format=Format.MESSAGE_FORMAT)
   String noLookupInterface(Class<?> clazz);
   
   @Message(id = BASE + 70, value = "{0} will be used for {1} lookup", format=Format.MESSAGE_FORMAT)
   String typeWillBeUsedForLookup(Type type, Class<?> clazz);

   @Message(id = BASE + 75, value = "Unable to lookup BeanManager.")
   String unableToLookupBeanManager();
   
   @Message(id = BASE + 80, value = "Unable to obtain BeanManager from {0}", format=Format.MESSAGE_FORMAT)
   String unableToObtainBeanManager(String name);

   @Message(id = BASE + 85, value = "Unable to obtain ResteasyCdiExtension instance.")
   String unableToObtainResteasyCdiExtension();
   
   @Message(id = BASE + 90, value = "Unable to perform JNDI lookups. You are probably running on GAE.")
   String unableToPerformJNDILookups();
   
   @Message(id = BASE + 95, value = "Using CdiConstructorInjector for class {0}.", format=Format.MESSAGE_FORMAT)
   String usingCdiConstructorInjector(Class<?> clazz);
   
   @Message(id = BASE + 100, value = "Using {0} for lookup of Session Bean {1}.", format=Format.MESSAGE_FORMAT)
