...
image:https://github.com/resteasy/resteasy/actions/workflows/maven.yml/badge.svg?branch=main[Github CI,link=https://github.com/resteasy/resteasy/actions/workflows/maven.yml?query=branch%3Amain]
...
|===
| RESTEasy Version | Branch | Jakarta REST Spec | JDK Required | Status

| *8.0.x* | `main` | 5.0 (In Development) | 21+ | Active Development
| *7.0.x* | `7.0` | 4.0 | 17+ | Active / Bug Fixes
| *6.2.x* | `6.2` | 3.1 | 11+ | Maintenance / Security Only
|===
...
=== Changes Requiring an Unreleased Jakarta REST API

Some changes cannot build against a released version of the Jakarta REST API, either because they
implement a specification change which is still under review, or one which is merged but not yet
released. Note this in the pull request description so the `requires-spec-snapshot` label can be
applied.

If the change implements a specific Jakarta REST pull request, include a line like the following in
the description:

----
Implements jakartaee/rest#1234
----

This links the two pull requests together and is also read by CI, which then builds and tests
RESTEasy against that pull request. Once it is merged, or if there is no pull request to reference,
the specification's `main` branch is used instead. The line does not need updating after the
specification change is merged.

A pull request labeled `requires-spec-snapshot` is converted to a draft and cannot be merged until
the Jakarta REST API it needs is released and the label is removed. The usual builds are skipped
while the label is present, and the
link:.github/workflows/jakarta-rest-snapshot-pr.yml[Test against Jakarta REST Spec] workflow takes
their place.
