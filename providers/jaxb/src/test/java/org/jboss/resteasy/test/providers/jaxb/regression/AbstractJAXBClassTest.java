...
public class AbstractJAXBClassTest
{
   ...
   public void testSeeAlso() throws Exception
   {
      ...
      Kunde kunde = (Kunde) ctx.createUnmarshaller().unmarshal(new ByteArrayInputStream(kundeXml.getBytes()));
      System.out.println("HERE KUNDE!!!!" + kunde.getNachname());
   ...
   {
      {
         ...
         try
         {
            method.setRequestEntity(new StringRequestEntity("<?xml version=\"1.0\"?><person><name>bill</name></person>", "application/xml", null));
      ...
      {
         HttpClient client = new HttpClient();
         PostMethod method = new PostMethod("http://localhost:8081/kunde");
         try
         {
