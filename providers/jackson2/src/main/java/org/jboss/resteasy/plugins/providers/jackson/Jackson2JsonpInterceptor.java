 ...
 * </p>
 * <p>
 * Jackson JSON processor can produce such an response. This interceptor checks if the media type is a JavaScript one if there
 * is a query
 * parameter with the method name. The default name of this query parameter is "callback". So this interceptor is compatible
 * with
...
public class Jackson2JsonpInterceptor implements AsyncWriterInterceptor {
    ...
    public void aroundWriteTo(WriterInterceptorContext context) throws IOException, WebApplicationException {
        ...
        if (enabled && function != null && !function.trim().isEmpty()
                && !jsonpCompatibleMediaTypes.getPossible(context.getMediaType()).isEmpty()) {
    ...
    /**
     * Set an fix {@link ObjectMapper}. If this is not set {@link Providers} are used for lookup. If there are is none too, use
     * a default one.
     *
     * @param objectMapper
     */
