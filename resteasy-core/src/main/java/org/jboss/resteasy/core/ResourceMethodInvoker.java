...
public class ResourceMethodInvoker implements ResourceInvoker, JaxrsInterceptorRegistryListener {
    ...
    public ResourceMethodInvoker(final ResourceMethod method, final InjectorFactory injector, final ResourceFactory resource,
        ...
        requestFilters = resourceMethodProviderFactory.getContainerRequestFilterRegistry()
                .postMatch(method.getResourceClass().getClazz(), method.getAnnotatedMethod());
        responseFilters = resourceMethodProviderFactory.getContainerResponseFilterRegistry()
                .postMatch(method.getResourceClass().getClazz(), method.getAnnotatedMethod());
        ...
        ContextResolver<GeneralValidator> resolver = providerFactory.getContextResolver(GeneralValidator.class,
        ...
        hasProduces = method.getMethod().isAnnotationPresent(Produces.class)
                || method.getMethod().getClass().isAnnotationPresent(Produces.class);
    }
    ...
    protected BuiltResponse invokeOnTarget(HttpRequest request, HttpResponse response, Object target) {
        ...
        try {
            ...
            if (resource instanceof SingletonResource) {
                tracingLogger.logDuration("METHOD_INVOKE", timestamp, ((SingletonResource) resource).traceInfo(),
                        method.getMethod());
            } else {
    ...
    private BuiltResponse handleInvocationException(AsyncResponseConsumer asyncStreamResponseConsumer, HttpRequest request,
            RuntimeException ex) {
    ...
    public boolean doesConsume(MediaType contentType) {
        ...
        if (method.getConsumes().length == 0 || (contentType == null && !expectsBody))
            return true;
    ...
    protected MediaType resolveContentTypeByAccept(List<MediaType> accepts, Object entity) {
        ...
        for (MediaType accept : accepts) {
            if (resourceMethodProviderFactory.getMessageBodyWriter(clazz, type, method.getAnnotatedMethod().getAnnotations(),
                    accept) != null) {
                return accept;
            }
        }
