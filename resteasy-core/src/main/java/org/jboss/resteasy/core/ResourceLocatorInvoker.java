...
public class ResourceLocatorInvoker implements ResourceInvoker {
    ...
    public ResourceLocatorInvoker(final ResourceFactory resource, final InjectorFactory injector,
    ...
    protected Object resolveTargetFromLocator(HttpRequest request, HttpResponse response, Object locator) {
        ...
        RuntimeException lastException = (RuntimeException) request
                .getAttribute(ResourceMethodRegistry.REGISTRY_MATCHING_EXCEPTION);
    ...
    public BuiltResponse invoke(HttpRequest request, HttpResponse response) {
        ...
        if (resource instanceof CompletionStage) {
            return ((CompletionStage<Object>) resource).thenApply(target -> invokeOnTargetObject(request, response, target))
    ...
    protected BuiltResponse invokeOnTargetObject(HttpRequest request, HttpResponse response, Object target) {
        if (target == null) {
            NotFoundException notFound = new NotFoundException(
                    Messages.MESSAGES.nullSubresource(request.getUri().getAbsolutePath()));
            throw notFound;
        }
