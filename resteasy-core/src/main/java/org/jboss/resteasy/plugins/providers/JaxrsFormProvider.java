...
public class JaxrsFormProvider implements MessageBodyReader<Form>, AsyncMessageBodyWriter<Form> {
    ...
    public Form readFrom(Class<Form> type, Type genericType, Annotation[] annotations, MediaType mediaType,
        ...
        MultivaluedMap<String, String> map = new FormUrlEncodedProvider().readFrom(null, null, annotations, mediaType,
    ...
    public CompletionStage<Void> asyncWriteTo(Form form, Class<?> type, Type genericType, Annotation[] annotations,
        ...
        LogMessages.LOGGER.debugf("Provider : %s,  Method : writeTo", getClass().getName());
        return new FormUrlEncodedProvider().asyncWriteTo(form.asMap(), null, null, annotations, mediaType, httpHeaders,
                entityStream);
    }
}
