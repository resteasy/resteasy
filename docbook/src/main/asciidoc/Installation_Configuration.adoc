...
[[_configuration_factory_resolver]]
===== Per-deployment factory resolution

By default, the selected `ConfigurationFactory` is cached statically (singleton behavior). This implementation works in
many cases, while it could cause classloader and deployment isolation issues in environments where multiple deployments
provide their own `ConfigurationFactory` implementations.

When the system property `dev.resteasy.configuration.factory.resolver.enabled` (`false` by default) is set to `true`,
`ConfigurationFactory.getInstance()` delegates to a `ConfigurationFactoryResolver` discovered via `ServiceLoader`.
The default resolver (`ResteasyConfigurationFactoryResolver` in `resteasy-core`) discovers `ConfigurationFactory`
implementations using the thread context classloader and caches the result per-request in `ResteasyContext`, enabling
per-deployment isolation.

Custom `ConfigurationFactoryResolver` implementations can be registered via `ServiceLoader`. When multiple resolvers
are present, the one with the lowest `priority()` value is selected (default priority is 100).

NOTE: Containers like WildFly that manage their own `ConfigurationFactory` lifecycle should leave this property
disabled (the default). The resolver mechanism is intended for environments like embedded servers (e.g., via
`SeBootstrap`) or custom containers that do not pre-populate the `ResteasyContext`.
...
| `dev.resteasy.image.threshold`
| 200MB
| The maximum estimated size of a decoded image, including its thumbnails, which the
  `org.jboss.resteasy.plugins.providers.IIOImageProvider` will read. Images whose estimated size exceeds this value are
  rejected with a 400 before being decoded. +
  Please note this is the estimated size of the decoded image in memory, not the size of the request body, and it is a
  per-image estimate rather than a limit on the memory used across concurrent requests. A value of `-1` disables the
  validation. +
  The value is a number followed by an optional unit of `B`, `KB`, `MB`, `GB`, `TB`, `PB`, `EB` or `ZB`. The unit is
  case-insensitive and may be separated from the number by a space, so `200MB`, `200mb` and `200 MB` are equivalent.
  The units are binary, making `200MB` 209715200 bytes. A value with no unit, or with a unit other than those listed,
  is read as a number of bytes; `200` and `200MiB` both mean 200 bytes rather than 200 megabytes.
|===
